...
export default function WorkspaceHighlightsPage() {
  const { boards }                          = useOutletContext();
  const { currentWorkspace, starredBoards, trackRecent } = useNav();
  ...
  const CardRow = ({ card, accent }) => (
    <Link to={`/board/${card.boardId}`} className="flex items-center gap-3 px-4 py-2.5 hover:bg-white/5 transition-colors rounded-lg group">
  ...
  return (
    <div className="p-8 space-y-8">
      <p className="text-xs text-[#8C9BAB] uppercase tracking-wider font-semibold">{currentWorkspace.name}</p>
      <h1 className="text-white font-bold text-xl -mt-6">Highlights</h1>
      ...
      <section>
        ...
        <div className="bg-white/5 border border-white/10 rounded-xl overflow-hidden">
          {RECENT_ACTIVITY.map((item, i) => (
            <div key={item.id} className={`flex items-start gap-3 px-4 py-3 ${i < RECENT_ACTIVITY.length - 1 ? 'border-b border-white/5' : ''}`}>
      ...
      <section>
        ...
        {starredBoards.length === 0 ? (
          ...
          <div className="flex flex-wrap gap-3">
            {starredBoards.map(b => (
              <Link key={b.id} to={`/board/${b.id}`} onClick={() => trackRecent(b)}
      ...
      <section>
        ...
        {loading ? <p className="text-[#8C9BAB] text-sm">Loading…</p>
          : dueSoon.length === 0 ? <p className="text-[#8C9BAB] text-sm">No cards due in the next 3 days.</p>
          : <div className="bg-white/5 border border-white/10 rounded-xl overflow-hidden">{dueSoon.map(c => <CardRow key={c.id} card={c} accent="bg-orange-400" />)}</div>
        }
      </section>
      ...
      <section>
        ...
        {loading ? <p className="text-[#8C9BAB] text-sm">Loading…</p>
          : overdue.length === 0 ? <p className="text-[#8C9BAB] text-sm">No overdue cards. Great job! 🎉</p>
          : <div className="bg-white/5 border border-white/10 rounded-xl overflow-hidden">{overdue.map(c => <CardRow key={c.id} card={c} accent="bg-red-400" />)}</div>
        }
      </section>
    </div>
  );
